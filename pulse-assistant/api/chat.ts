...
Be helpful, concise, and friendly. Match user's language (English, Hindi, Hinglish, Marathi, etc.).`;
    ...
    if (isBDE) {
      ...
      let debugError = "";
      try {
        const { data: bdeData, error: bdeError } = await ctx.supabase
          .from("bde_sequences")
          .insert({
            generated_by: generatedBy,
            generated_by_email: generatedByEmail,
            client_name: clientName,
            linkedin_post: userMessage.slice(0, 2000),
            full_sequence: replyText,
            metadata: { generated_at: new Date().toISOString() },
          })
          .select("id")
          .single();
        ...
        if (bdeError) {
          debugError = JSON.stringify(bdeError);
        } else if (bdeData?.id) {
          sequenceId = bdeData.id;
        }
      } catch (e) {
        debugError = String(e);
      }
