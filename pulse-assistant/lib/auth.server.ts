...
export async function authorizeToken(token: string, viewAs: string | null = null): Promise<AuthedContext> {
  // DEV BYPASS
  if (!token || token.split(".").length !== 3 || token === "dev-bypass") {
    const SUPABASE_URL = process.env.SUPABASE_URL!;
    const SUPABASE_PUBLISHABLE_KEY = process.env.SUPABASE_PUBLISHABLE_KEY!;
    const supabase = createClient<Database>(SUPABASE_URL, SUPABASE_PUBLISHABLE_KEY);
    return { supabase, userId: "dev-user", actingUserId: "dev-user", isSuperAdmin: true };
  }
