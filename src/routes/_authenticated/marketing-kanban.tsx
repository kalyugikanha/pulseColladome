...
function MarkDoneDialog({ task, onClose, onConfirm }: {
  task: KanbanTask | null; onClose: () => void;
  onConfirm: (v: { hours: number; note?: string }) => void;
}) {
  const [hours, setHours] = useState<string>("");
  const [note, setNote] = useState<string>("");
  useEffect(() => { setHours(""); setNote(""); }, [task]);
        ...
        <div className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <Input type="number" min={0} step={0.25} value={hours}
              onChange={(e) => setHours(e.target.value)} placeholder="e.g. 3.5" autoFocus />
          </div>
          <div className="space-y-1">
            <Label>Note for the creator (optional)</Label>
            <Textarea rows={2} value={note} onChange={(e) => setNote(e.target.value)} placeholder="Anything they should know before approving?" />
          </div>
          <p className="text-xs text-muted-foreground">
            The task creator will review these hours. They only land in your timesheet once approved.
          </p>
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Cancel</Button>
          <Button className="gradient-primary" disabled={!valid}
            onClick={() => onConfirm({ hours: hoursNum, note: note || undefined })}>
            Send for approval
          </Button>
        </DialogFooter>
      </DialogContent>
