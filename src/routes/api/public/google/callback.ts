    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        try {
          ...
          return html(
            `<h2 class="ok">Google Calendar connected</h2><p>${email ?? ""}</p><p>You can close this window.</p>`,
          );
        } catch (e: unknown) {
