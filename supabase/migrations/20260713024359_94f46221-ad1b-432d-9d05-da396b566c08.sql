...
CREATE OR REPLACE FUNCTION public.is_non_working_day(_d date)
RETURNS boolean
LANGUAGE sql
STABLE
...
AS $$
  SELECT
    -- Sunday
    EXTRACT(ISODOW FROM _d)::int = 7
    -- 2nd or 4th Saturday
    OR (EXTRACT(ISODOW FROM _d)::int = 6
        AND ((EXTRACT(DAY FROM _d)::int - 1) / 7 + 1) IN (2, 4))
    -- Seeded holiday
    OR EXISTS (SELECT 1 FROM public.holidays h WHERE h.holiday_date = _d);
$$;
