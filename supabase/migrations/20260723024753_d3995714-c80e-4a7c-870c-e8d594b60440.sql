...
BEGIN
  ...
  LOOP
    ...
    BEGIN
      ...
      -- Propagate the template's current reviewer forward. Update reviewer_id
      -- only (not assignee_id) so trg_tasks_auto_reviewer — scoped to
      -- UPDATE OF assignee_id — does not re-fire and clobber this value.
